...
--[[
*	Collection of utility/extension SCAR functions, written by Janne252
...
function Lib_SetupMod(guid, name)
	g_guid = guid
	g_modName = name
end

function Mod_GetIcon(icon) 
	return "ModIcons_"..g_guid.."_"..icon
end

function Mod_GetAbilityBlueprint(blueprint)
	return BP_GetAbilityBlueprint(g_guid .. ":" .. blueprint)
end

function Mod_GetSquadBlueprint(blueprint)
	return BP_GetSquadBlueprint(g_guid .. ":" .. blueprint)
end

function Mod_GetEntityBlueprint(blueprint)
	return BP_GetEntityBlueprint(g_guid .. ":" .. blueprint)
end

function Mod_GetUpgradeBlueprint(blueprint)
	return BP_GetUpgradeBlueprint(g_guid .. ":" .. blueprint)
end

function Msg(text)
	if g_enable_messages then
		text = String_Replace(text, g_guid, g_modName)
		...
		g_text = text.."\n"..g_text
		dr_clear("DEBUGPRINT")
		dr_setautoclear("DEBUGPRINT", 0)
		dr_text2d("DEBUGPRINT", 0.615, 0.025, g_text, 0, 255, 0)
	end
end

function Debug_SetMessagesEnabled(enable)
	g_enable_messages = enable
end
...
function String_Replace(s, find, replace)
	local str = s
	if replace == nil then
		replace = ""
	end
	if scartype(find) ~= ST_TABLE then
		find = {find}
	end
	
	if scartype(replace) ~= ST_TABLE then
		replace = {replace}
	end
	
	for key, findStr in ipairs(find) do
		local replaceStr = replace[key]
		str = string.gsub(str, findStr, replaceStr)
	end
	return str
end
...
function Team_GetRandomPlayers(team, amount)
	local players = {}
	local result = {}
	...
	return Table_GetRandomItem(players, 2)
end
...
function Util_GetBPName(item)
	local st = scartype(item)
	local bp = item
	if st == ST_SQUAD then
		bp = Squad_GetBlueprint(item)
	elseif st == ST_ENTITY then
		bp = Entity_GetBlueprint(item)
	end
	-- Critical, Ability, Upgrade, etc. can be directly processed with BP_GetName
	return BP_GetName(bp)
end
...
-- Strat points give new units to attackers
-- Radio towers give new units and artillery to defenders
-- wincondition: all strat points captured by attackers
-- Capturing radio towers gives extra time to attackers

-- Win Condition Attackers: Capture and hold  ALL 3 Strategic Points.
-- Win Condition Defenders: Hold at least 1 strategic point until the time is up.
-- Additional Condition Defenders: Losing all radio towers will cripple manpower income by 30%.

-- emergency assault, attackers: [X]
-- emergency help, defenders: [X]
-- attacker reinforcements: [X]
-- defender reinforcements: [X]
-- defender defensive air support: [X]
-- 30% less mp if all radios lost [ ]
-- winconditions [ ]
-- 18:49 - O'Field: 60 popcap for defender -60 mp base income, attackers 75 popcap

function AC_Init()
	Util_CreateEntities(nil, eg_support_weapons, EBP.GERMAN.HOWITZER_105MM_LE_FH18_MP, mkr_lefh_01, 1 )
	Util_CreateEntities(nil, eg_support_weapons, EBP.GERMAN.HOWITZER_105MM_LE_FH18_MP, mkr_lefh_02, 1 )
	Util_CreateEntities(nil, eg_support_weapons, EBP.GERMAN.HOWITZER_105MM_LE_FH18_MP, mkr_lefh_03, 1 )
	Util_CreateEntities(nil, eg_support_weapons, EBP.GERMAN.PAK43_88MM_AT_GUN_MP, mkr_pak43_01, 1 )
	Util_CreateEntities(nil, eg_support_weapons, EBP.GERMAN.PAK43_88MM_AT_GUN_MP, mkr_pak43_02, 1 )
	EGroup_SetInvulnerable(eg_inv, true)
-- List all vehicles to abandon in this table. Use comma as separator.
	local t_AbandondedVehicles = {sg_panzer_iv, sg_tiger}

	local player = World_GetPlayerAt(1)
	
	for key, sgroup in ipairs(t_AbandondedVehicles) do
		Command_PlayerSquadCriticalHit(player, sgroup, PCMD_CriticalHit, BP_GetCriticalBlueprint("vehicle_lose_treads_or_wheels"), 1, false)
	end
	...
	g_defending_team = 1
	g_attacking_team = 0
	g_objectives_running = false
	g_mission_ended = false
	g_all_radio_towers_lost_defenders = false
	g_attackers_captured_radio_tower_once = false
	g_defenders_one_radio_tower_left_first_time = false
	g_radio_tower_penalty_modifiers = {}
	g_hint_radio_tower_time_bonus = {}
	world = "world"
	Debug_SetMessagesEnabled(false)
	Lib_SetupMod("5b0852a1dee6406aa892aa8d9c7211c2", "AC")
	...
	ATTACKER_BONUS = {
		LIGHT_REINFORCEMENTS = 1,
		MEDIUM_REINFORCEMENTS = 2,
	}
	
	DEFENDER_EMERGENCY_HELP = {
		AERIAL_DEFENCE = 1,
		TIGER_TANK = 2,
		SECTOR_ARTILLERY = 3,
	}
	AC_Config = {
		mission_time = 30 * 60,
		bonus_time_per_radio_capture = 10 * 60,
		attacker_emergency_time = 10 * 60,
		attacker_populationCap = 75,
		defender_populationCap = 60,
		defender_manpowerAddition = -0.125,
		...
		attacker_medium_reinforcements = {
		...
		defender_aerial_attack_positions = {
			mkr_stuka_01, mkr_stuka_02, mkr_stuka_03
		},
		defender_aerial_attack_abilities = {
			{ability = ABILITY.GERMAN.STUKA_CLOSE_AIR_SUPPORT, upgrade = UPG.GERMAN.AERIAL_SUPERIORITY_STUKA_CLOSE_AIR_SUPPORT}, 
			{ability = ABILITY.GERMAN.STUKA_STRAFING_RUN, upgrade = UPG.GERMAN.AERIAL_SUPERIORITY_STUKA_STRAFE},
		},
		attacker_emergency_para_assault_positions = {
			mkr_para_01, mkr_para_02, mkr_para_03, mkr_para_04, mkr_para_05, mkr_para_06,
		},
		attacker_emergency_arty_positions = {
			mkr_emergency_arty_01, mkr_emergency_arty_02, mkr_emergency_arty_03, mkr_emergency_arty_04, mkr_emergency_arty_05, mkr_emergency_arty_06
		},
	
	}
	g_strategic_points = {
		{
			egroup = eg_strategic_point_01,
			entity = EGroup_GetSpawnedEntityAt(eg_strategic_point_01, 1),
			owner = world,
			attacker_held_seconds = 0,
			attacker_bonus_held_time = 5 * 60,
			attacker_bonus = ATTACKER_BONUS.LIGHT_REINFORCEMENTS,
		},
	
		{
			egroup = eg_strategic_point_02,
			entity = EGroup_GetSpawnedEntityAt(eg_strategic_point_02, 1),
			owner = world,
			attacker_held_seconds = 0,
			attacker_bonus_held_time = 5 * 60,
			attacker_bonus = ATTACKER_BONUS.MEDIUM_REINFORCEMENTS,
		},
	
		{
			egroup = eg_strategic_point_03,
			entity = EGroup_GetSpawnedEntityAt(eg_strategic_point_03, 1),
			owner = world,
			attacker_held_seconds = 0,
			attacker_bonus_held_time = 1 * 60,
			attacker_bonus = nil,
	...
	g_radio_towers = {
		{
			egroup = eg_radio_tower_01,
			entity = EGroup_GetSpawnedEntityAt(eg_radio_tower_01, 1),
			owner = world,
			defender_held_seconds = 0,
			defender_bonus_held_time = 2 * 60,
			defender_bonus = DEFENDER_BONUS.AERIAL_DEFENCE,
			first_capture_attacker_bonus_granted = false,
			defender_emergency_help = DEFENDER_EMERGENCY_HELP.AERIAL_DEFENCE,
		},
	
		{
			egroup = eg_radio_tower_02,
			entity = EGroup_GetSpawnedEntityAt(eg_radio_tower_02, 1),
			owner = world,
			defender_held_seconds = 0,
			defender_bonus_held_time = 5 * 60,
			defender_bonus = DEFENDER_BONUS.REINFORCEMENTS,
			first_capture_attacker_bonus_granted = false,
			defender_emergency_help = DEFENDER_EMERGENCY_HELP.TIGER_TANK,
		},
	
		{
			egroup = eg_radio_tower_03,
			entity = EGroup_GetSpawnedEntityAt(eg_radio_tower_03, 1),
			owner = world,
			defender_held_seconds = 0,
			defender_bonus_held_time = 3 * 60,
			defender_bonus = DEFENDER_BONUS.SECTOR_ARTILLERY,
			first_capture_attacker_bonus_granted = false,
			defender_emergency_help = DEFENDER_EMERGENCY_HELP.SECTOR_ARTILLERY,
		},
	}
	
	AC_ObjectiveInit()
	Rule_AddInterval(AC_OneSecondTick, 1)
	Rule_Add(AC_Monitor)
	AC_SetupHints()
	AC_SetupPlayers()
end

function AC_SetupHints()
	Team_ExecuteLocally(g_attacking_team, function()
		HintPoint_Add(eg_strategic_point_01, true, Util_CreateLocString("Hold to receive light reinforcements."))
		HintPoint_Add(eg_strategic_point_02, true, Util_CreateLocString("Hold to receive medium reinforcements."))
		

		local hint_radio01 = HintPoint_Add(eg_radio_tower_01, true, Util_CreateLocString("Capture to receive additional time."))
		local hint_radio02 = HintPoint_Add(eg_radio_tower_02, true, Util_CreateLocString("Capture to receive additional time."))
		local hint_radio03 = HintPoint_Add(eg_radio_tower_03, true, Util_CreateLocString("Capture to receive additional time."))
		
		g_hint_radio_tower_time_bonus = {hint_radio01, hint_radio02, hint_radio03}

	end)
	Team_ExecuteLocally(g_defending_team, function()
		HintPoint_Add(eg_radio_tower_01, true, Util_CreateLocString("Hold to receive air support."))
		HintPoint_Add(eg_radio_tower_02, true, Util_CreateLocString("Hold to receive reinforcements."))
		HintPoint_Add(eg_radio_tower_03, true, Util_CreateLocString("Hold to receive artillery support."))
	end)	
end

function AC_SetupPlayers()
	Players_ForEachInTeam(g_attacking_team, function(pid, idx, player)
		Player_SetPopCapOverride(player, AC_Config.attacker_populationCap)
	end)
	Players_ForEachInTeam(g_defending_team, function(pid, idx, player)
		Player_SetPopCapOverride(player, AC_Config.defender_populationCap)
		Modify_PlayerResourceRate(player, RT_Manpower, AC_Config.defender_manpowerAddition, MUT_Addition)
	end)
end

function Player_GetRole(player)
	local team = Player_GetTeam(player)
	if team == g_defending_team then
		return AC_PLAYER_ROLE.DEFENDER
	elseif team == g_attacking_team  then
		return AC_PLAYER_ROLE.ATTACKER
	end
end

function AC_Monitor()
	
	for key, radioTower in ipairs(g_radio_towers) do
		-- deal with neutral ones
		local owner = world
		local isNeutral = World_OwnsEntity(radioTower.entity)
		if not isNeutral then
			owner = Entity_GetPlayerOwner(radioTower.entity)
		end
		
		if owner ~= radioTower.owner and not isNeutral then
			local role = Player_GetRole(owner)
			if role == AC_PLAYER_ROLE.ATTACKER and not g_attackers_captured_radio_tower_once then
				g_attackers_captured_radio_tower_once = true
			end
			if role == AC_PLAYER_ROLE.ATTACKER and not radioTower.first_capture_attacker_bonus_granted then
				Attackers_AddTime(AC_Config.bonus_time_per_radio_capture)
				Team_ExecuteLocally(g_attacking_team, function()
					Msg("Removed hintPoint")
					HintPoint_SetVisible(g_hint_radio_tower_time_bonus[key], false)
				end)
				
				radioTower.first_capture_attacker_bonus_granted = true
			end
			
			radioTower.owner = owner
		end
		
	end
end

function AC_GameOver(winningTeam)
if not g_mission_ended then
local losingTeam = Team_GetEnemyTeam(winningTeam)
...
g_mission_ended = true
end
end

	
function AC_OneSecondTick()
	if g_objectives_running then
		AC_Config.mission_time = AC_Config.mission_time - 1
		
		if AC_Config.mission_time == 0 then
			AC_GameOver(g_defending_team)
			
		elseif AC_Config.mission_time < AC_Config.attacker_emergency_time and not g_attackers_captured_radio_tower_once then
			g_attackers_captured_radio_tower_once = true
			
			Attacker_EmergencySituation()
		end
		
		local ownedByAttackers = 0
		for key, stratPoint in ipairs(g_strategic_points) do
			if not World_OwnsEntity(stratPoint.entity) then
				local owner = Entity_GetPlayerOwner(stratPoint.entity)
				if Player_GetRole(owner) == AC_PLAYER_ROLE.ATTACKER then
					ownedByAttackers = ownedByAttackers + 1
				end
			end
		end
		
		if ownedByAttackers == 3 then
			AC_GameOver(g_attacking_team)
		end
		
		local ownedByAttackers = 0
		for key, radioTower in ipairs(g_radio_towers) do
			if not World_OwnsEntity(radioTower.entity) then
				local owner = Entity_GetPlayerOwner(radioTower.entity)
				if Player_GetRole(owner) == AC_PLAYER_ROLE.ATTACKER then
					ownedByAttackers = ownedByAttackers + 1
				end
			end
		...
		if ownedByAttackers == 3 and not g_all_radio_towers_lost_defenders then
			g_all_radio_towers_lost_defenders = true
			Players_ForEachInTeam(g_defending_team, function(pid, idx, player)
				local modId = Modify_PlayerResourceRate(player, RT_Manpower, 2/3)
				table.insert(g_radio_tower_penalty_modifiers, modId)
			end)
		elseif ownedByAttackers < 3 and g_all_radio_towers_lost_defenders then
			for key, modId in ipairs(g_radio_tower_penalty_modifiers) do
		...
		local ownedByDefenders = 0
		for key, radioTower in ipairs(g_radio_towers) do
			if not World_OwnsEntity(radioTower.entity) then
				local owner = Entity_GetPlayerOwner(radioTower.entity)
				if Player_GetRole(owner) == AC_PLAYER_ROLE.DEFENDER then
					ownedByDefenders = ownedByDefenders + 1
				end
			end
		end	
		if ownedByDefenders == 1 and not g_defenders_one_radio_tower_left_first_time then
			g_defenders_one_radio_tower_left_first_time = true
			Defender_EmergencySituation()
		end	
	...
	for key, radioTower in ipairs(g_radio_towers) do
		...
		if not isNeutral then
			owner = Entity_GetPlayerOwner(radioTower.entity)
			local role = Player_GetRole(owner)
			if role == AC_PLAYER_ROLE.DEFENDER then
				radioTower.defender_held_seconds = radioTower.defender_held_seconds + 1
				
				if radioTower.defender_held_seconds == radioTower.defender_bonus_held_time then
					if radioTower.defender_bonus == DEFENDER_BONUS.AERIAL_DEFENCE then
						local player = Team_GetFirstPlayer(g_defending_team)
						local target = Table_GetRandomItem(AC_Config.defender_aerial_attack_positions)
						local item = Table_GetRandomItem(AC_Config.defender_aerial_attack_abilities)
						Player_AddAbility(player, item.ability)
						Cmd_Ability(player, item.ability, target, nil, true, false)
						
						Msg("called stuka attack (defenders) ")
						Msg("Defender radioTower time bonus: Aerial Defence")
						
					elseif radioTower.defender_bonus == DEFENDER_BONUS.REINFORCEMENTS then
						local players = Team_GetRandomPlayers(g_defending_team, 2)
						
						for key, player in ipairs(players) do
							local sbp = Table_GetRandomItem(AC_Config.defender_reinforcements)
							local squad = Squad_CreateAndSpawnToward(sbp, player, 0, Marker_GetPosition(mkr_reinforcement_point_defenders), World_Pos(0, 0, 0))
							Cmd_Move(Squad_GetTempSGroup(squad), Player_GetStartingPosition(player))
							Player_ExecuteLocally(player, function()
								Util_GlobalMessage(Util_CreateLocString("Reinforcements have arrived!"), 5)
								UI_CreateMinimapBlip(Marker_GetPosition(mkr_reinforcement_point_defenders), 5, BT_General)
							end)
							Msg("Defender radioTower time bonus: Reinforcements")
						end
					elseif radioTower.defender_bonus == DEFENDER_BONUS.SECTOR_ARTILLERY then
						local player = Team_GetFirstPlayer(g_defending_team)
						Player_AddAbility(player, ABILITY.WEST_GERMAN.ZEROING_ARTILLERY)
						Player_AddAbility(player, ABILITY.WEST_GERMAN.FLARE_ARTILLERY)
						local pos = Table_GetRandomItem(AC_Config.defender_aerial_attack_positions)
						Cmd_Ability(player, ABILITY.WEST_GERMAN.ZEROING_ARTILLERY, pos, nil, true, false)
						Cmd_Ability(player, ABILITY.WEST_GERMAN.FLARE_ARTILLERY, pos, nil, true, false)
						Msg("Defender radioTower time bonus: Zeroing Artillery")
					end
					radioTower.defender_held_seconds = 0
				end
			elseif role == AC_PLAYER_ROLE.ATTACKER then
				radioTower.defender_held_seconds = 0
			end
	...
	for key, stratPoint in ipairs(g_strategic_points) do
		local owner = world
		local isNeutral = World_OwnsEntity(stratPoint.entity)
		if not isNeutral then
			owner = Entity_GetPlayerOwner(stratPoint.entity)
			local role = Player_GetRole(owner)
			if role == AC_PLAYER_ROLE.ATTACKER then
				stratPoint.attacker_held_seconds = stratPoint.attacker_held_seconds + 1
				
				if stratPoint.attacker_held_seconds == stratPoint.attacker_bonus_held_time then
					if stratPoint.attacker_bonus == ATTACKER_BONUS.LIGHT_REINFORCEMENTS then
						local players = Team_GetRandomPlayers(g_attacking_team, 2)
						
						for key, player in ipairs(players) do
							local sbp = Table_GetRandomItem(AC_Config.attacker_light_reinforcements)
							local sbp2 = Table_GetRandomItem(AC_Config.attacker_light_reinforcements)
							local squad = Squad_CreateAndSpawnToward(sbp, player, 0, Marker_GetPosition(mkr_reinforcement_point_attackers), World_Pos(0, 0, 0))
							local squad2 = Squad_CreateAndSpawnToward(sbp2, player, 0, Marker_GetPosition(mkr_reinforcement_point_attackers), World_Pos(0, 0, 0))
							Cmd_Move(Squad_GetTempSGroup(squad), Player_GetStartingPosition(player))
							Cmd_Move(Squad_GetTempSGroup(squad2), Player_GetStartingPosition(player))
							Player_ExecuteLocally(player, function()
								Util_GlobalMessage(Util_CreateLocString("Reinforcements have arrived!"), 5)
								UI_CreateMinimapBlip(Marker_GetPosition(mkr_reinforcement_point_attackers), 5, BT_General)
							end)
							Msg("Attacker radioTower time bonus: Light Reinforcements")
						end
						
					elseif stratPoint.attacker_bonus == ATTACKER_BONUS.MEDIUM_REINFORCEMENTS then
						local players = Team_GetRandomPlayers(g_attacking_team, 2)
						
						for key, player in ipairs(players) do
							local sbp = Table_GetRandomItem(AC_Config.attacker_medium_reinforcements)
							local squad = Squad_CreateAndSpawnToward(sbp, player, 0, Marker_GetPosition(mkr_reinforcement_point_attackers), World_Pos(0, 0, 0))
							Cmd_Move(Squad_GetTempSGroup(squad), Player_GetStartingPosition(player))
							Player_ExecuteLocally(player, function()
								Util_GlobalMessage(Util_CreateLocString("Reinforcements have arrived!"), 5)
						...
						Msg("Attacker radioTower time bonus: Medium Reinforcements")
					--elseif stratPoint.defender_bonus == ATTACKER_BONUS.SECTOR_ARTILLERY then
						
						
					end
					stratPoint.attacker_held_seconds = 0
				end
			elseif role == AC_PLAYER_ROLE.ATTACKER then
				stratPoint.attacker_held_seconds = 0
			end
...
function Attacker_EmergencySituation()
	Msg("Attackers Emergency")
	--mkr_emergency_arty
-- Emergency Case: If the timer drops below 10 minutes before any radio tower has 
-- 
-- been captured once, emergency reinforcements will be deployed to the battlefield.
-- 
-- Radio Tower 2 will be saturated with artillery fire. ( sector artillery on enemy sector 
-- 
-- possible, maybe zeroing artillery?)
-- 
--  Paratrooper combat groups will be dropped on radio tower 1 or 3.	

-- 17:25 - O'Field: when time drops below 10 mins, and no radio tower has been captured, 
--	2 paratrooper combat groups drop on 2 of the 6 markers and that american 240mm arty saturates the 2nd radio tower at the marker location mkr_emergency_arty
-- 17:25 - O'Field: mkr_para_01-06
-- 17:25 - O'Field: for paratroopers
	local random_players = Team_GetRandomPlayers(g_attacking_team, 2)
	local random_positions = Table_GetRandomItem(AC_Config.attacker_emergency_para_assault_positions, 2)
	for key, pos in ipairs(random_positions) do
		local player = random_players[key]
		Player_AddAbility(player, ABILITY.AEF.AIR_DROP_COMBAT_GROUP)
		Player_SetPopCapOverride(player, 150)
		Cmd_Ability(player, ABILITY.AEF.AIR_DROP_COMBAT_GROUP, pos, nil, true, false)
		Util_Delay(480, function() Player_SetPopCapOverride(player, 75) end)
-- 480 ticks = 60 seconds
	end
	
	local player = Team_GetFirstPlayer(g_attacking_team)
	Player_AddAbility(player, ABILITY.AEF.MAJOR_ARTILLERY)
	for key, mkr in ipairs(AC_Config.attacker_emergency_arty_positions) do
		Cmd_Ability(player, ABILITY.AEF.MAJOR_ARTILLERY, mkr, nil, true, false)	
	end
	Team_ExecuteLocally(g_attacking_team, function()
		Util_GlobalMessage(Util_CreateLocString("Emergency assault started!"), 5)
	end)
end

function Defender_EmergencySituation()
-- Emergency Case: If the Enemy is holding 2 Radio Towers for the first time, the 
-- 
-- remaining Radio Tower will provide emergency support, depending on its typ.
-- 
-- Radio Tower 1: Stukas AT loitering Radio tower 2 and 3.
-- 
-- Radio Tower 2: Tiger Tank for a randomly picked player of the defenders.
-- 
-- Radio Twoer 3: Sector Artillery on all controlled Sectors.
	for key, radioTower in ipairs(g_radio_towers) do
		if not World_OwnsEntity(radioTower.entity) then
			local owner = Entity_GetPlayerOwner(radioTower.entity)
			local role = Player_GetRole(owner)
			...
			if role == AC_PLAYER_ROLE.DEFENDER then
				if radioTower.defender_emergency_help == DEFENDER_EMERGENCY_HELP.AERIAL_DEFENCE then
					Msg("Defender Emergecy: Stuka (1)")
					local player = Team_GetFirstPlayer(g_defending_team)
					Player_AddAbility(player, ABILITY.GERMAN.STUKA_CLOSE_AIR_SUPPORT)
					Util_Delay(1, function()
						Cmd_Ability(player, ABILITY.GERMAN.STUKA_CLOSE_AIR_SUPPORT, eg_radio_tower_02, nil, true, false)
						Msg("Called emergency stuka on radio 2")
					end)
					Util_Delay(8, function()
						Cmd_Ability(player, ABILITY.GERMAN.STUKA_CLOSE_AIR_SUPPORT, eg_radio_tower_03, nil, true, false)
						Msg("Called emergency stuka on radio 3")
					end)
					
					Team_ExecuteLocally(g_defending_team, function()
						Util_GlobalMessage(Util_CreateLocString("Emergency air support has started!"), 5)
						UI_CreateMinimapBlip(eg_radio_tower_02, 5, BT_General)
						UI_CreateMinimapBlip(eg_radio_tower_03, 5, BT_General)
					end)
				elseif radioTower.defender_emergency_help == DEFENDER_EMERGENCY_HELP.TIGER_TANK then
					Msg("Defender Emergecy: Tiger (2)")
					local player = Team_GetRandomPlayer(g_defending_team)
					local squad = Squad_CreateAndSpawnToward(SBP.GERMAN.TIGER_SQUAD_MP, player, 0, Marker_GetPosition(mkr_reinforcement_point_defenders), World_Pos(0, 0, 0))
					Cmd_Move(Squad_GetTempSGroup(squad), Player_GetStartingPosition(player))
					Team_ExecuteLocally(g_defending_team, function()
						Util_GlobalMessage(Util_CreateLocString("Emergency help has arrived!"), 5)
						UI_CreateMinimapBlip(Marker_GetPosition(mkr_reinforcement_point_defenders), 5, BT_General)
					end)
				
				elseif radioTower.defender_emergency_help == DEFENDER_EMERGENCY_HELP.SECTOR_ARTILLERY then
					Msg("Defender Emergecy: Sector Arty (3)")
					local player = Team_GetFirstPlayer(g_defending_team)
					EGroup_ForEach(eg_territory_sectors, function(egid, idx, entity)
						if not World_OwnsEntity(entity) then
							local owner = Entity_GetPlayerOwner(entity)
							if Player_GetRole(owner) == AC_PLAYER_ROLE.DEFENDER then
								Player_AddAbility(player, ABILITY.GERMAN.SECTOR_ARTILLERY)
								Cmd_Ability(player, ABILITY.GERMAN.SECTOR_ARTILLERY, Entity_GetPosition(entity), nil, true, false)
								
								Team_ExecuteLocally(g_defending_team, function()
									UI_CreateMinimapBlip(Entity_GetPosition(entity), 5, BT_General)
								end)						
							end
						end
					end)
					Team_ExecuteLocally(g_defending_team, function()
						Util_GlobalMessage(Util_CreateLocString("Emergency artillery has started!"), 5)
					end)	
				end
			end
		end
...
function AC_ObjectiveInit()
	OBJ_Attacker = {
		Title = Util_CreateLocString("Capture all Strategic Points"), Type = OT_Primary, Intel_Start = nil, Intel_Start_SkipFunc = nil,
		SetupUI = function() 
		end,	
		OnStart = function()
			Objective_StartTimer(OBJ_Attacker, COUNT_DOWN, AC_Config.mission_time)
			Util_Delay(8, function()
				local newTitle = Util_CreateLocString("Capture all Strategic Points in ")
				Objective_UpdateText(OBJ_Attacker, newTitle, newTitle, false)
			end)
			local uiElementTitle = Util_CreateLocString("Strategic Point")
			Util_Delay(4, function()
				Objective_AddUIElements(OBJ_Attacker, eg_strategic_point_01, true, uiElementTitle, false, nil, nil, HPAT_Objective)
			end)
			Util_Delay(8, function()
				Objective_AddUIElements(OBJ_Attacker, eg_strategic_point_02, true, uiElementTitle, false, nil, nil, HPAT_Objective)
			end)
			Util_Delay(12, function()
				Objective_AddUIElements(OBJ_Attacker, eg_strategic_point_03, true, uiElementTitle, false, nil, nil, HPAT_Objective)
			end)
		end,	
	...
	Objective_Register(OBJ_Attacker)
	OBJ_Defender = {
		Title = Util_CreateLocString("Hold at least one Strategic Point"), Type = OT_Primary, Intel_Start = nil, Intel_Start_SkipFunc = nil,
		SetupUI = function() 
		end,	
		OnStart = function()
			Objective_StartTimer(OBJ_Defender, COUNT_DOWN, AC_Config.mission_time)
			Util_Delay(8, function()
				local newTitle = Util_CreateLocString("Hold at least one Strategic Point for ")
				Objective_UpdateText(OBJ_Defender, newTitle, newTitle, false)
			end)
			local uiElementTitle = Util_CreateLocString("Strategic Point")
			Util_Delay(4, function()
				Objective_AddUIElements(OBJ_Attacker, eg_strategic_point_01, true, uiElementTitle, false, nil, nil, HPAT_Objective)
	...
	Objective_Register(OBJ_Defender)
	

	Team_ExecuteLocally(g_defending_team, function()
		Objective_Start(OBJ_Defender)
	end)
	Team_ExecuteLocally(g_attacking_team, function()
		Objective_Start(OBJ_Attacker)
	end)
	
	g_objectives_running = true
end

function Attackers_AddTime(addition)
	AC_Config.mission_time = AC_Config.mission_time + addition
	
	Util_GlobalMessage(Util_CreateLocString("Attacking Team gained " .. (addition / 60) .. " minutes more time!"), 5)
	
	Team_ExecuteLocally(g_attacking_team, function()
		Objective_StopTimer(OBJ_Attacker)
		Objective_StartTimer(OBJ_Attacker, COUNT_DOWN, AC_Config.mission_time)
	end)
	Team_ExecuteLocally(g_defending_team, function()
		Objective_StopTimer(OBJ_Defender)
		Objective_StartTimer(OBJ_Defender, COUNT_DOWN, AC_Config.mission_time)
	end)
end

Scar_AddInit(AC_Init)