...
function WinCondition_GameOver(winningTeam, losingTeam)
	...
	--[[	--causes trouble, probably 'cause it gets called too late
...
function WinCondition_Check()
	...
	for team,result in pairs(results) do
		if (result.surrender_count > 0 or result.annihilation_condition_count == 0) then
			...
			if winningTeam == Player_GetTeam(g_TigerPlayer) then Objective_Complete(obj_DestroyTiger) end
...
local function WinCondition_Init()
	...
	if g_TigerPlayer ~= nil then
		...
		obj_DestroyTiger = {
			...
			Title = Util_CreateLocString("Defend the Tiger"),
			Description = 0,
			Type = OT_Primary,
		}
...
-------------------------------------------------------------------------
		...
		for i = 1, World_GetPlayerCount() do
			...
			if team == Player_GetTeam(g_TigerPlayer) then
				LOC_reason = Util_CreateLocString("No further orders required")
			else
				LOC_reason = Util_CreateLocString("You have failed and as such will be replaced by someone more... qualified for the job")
			end
			Player_SetAllCommandAvailabilityInternal(player, ITEM_LOCKED, LOC_reason)
		end
...
--[[function g_EndSequence_axis()
	...
	SGroup_SetInvulnerable(sg_EndSequence, true)
	if g_camera_moved == false then
		g_camera_moved = true
		Camera_MoveTo(sg_EndSequence, true, 0.35, true, true)
	end
end]]
...
function TLT_updateTigerData()
	if SGroup_IsAlive(sg_Tiger) == true then
...
function TLT_GetAlliedStartingPosition()
	local g_found_entry_point = false

	for i = 1, World_GetPlayerCount() do
		local player = World_GetPlayerAt(i)
		
		if Player_GetTeam(player) ~= Player_GetTeam(g_TigerPlayer) and g_found_entry_point == false then
			g_found_entry_point = true
...
~ edit: after some testing I found out that it scar seems to not add the entry points into the table and thus the closest entry in the table is always 'nil' as the table is empty --]]
--[
